...
async def get_medicines(search: str = Query("", description="Search term for medicine name")):
...
async def create_medicine(medicine: Medicine):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Error creating medicine")
...
async def get_medicine(medicine_id: str):
    ...
    try:
        ...
        logging.error(f"Error fetching medicine: {e}")
        raise HTTPException(status_code=500, detail="Error fetching medicine")

@api_router.put("/medicines/{medicine_id}", response_model=Medicine)
async def update_medicine(medicine_id: str, medicine_data: dict):
    """Update a medicine"""
    try:
        existing_medicine = await db.medicines.find_one({"id": medicine_id})
        ...
        allowed_fields = ["name", "price", "stock_quantity", "expiry_date", "batch_number", "supplier", "barcode"]
        for field in allowed_fields:
            if field in medicine_data and medicine_data[field] is not None:
                update_data[field] = medicine_data[field]
        
        # Handle stock quantity changes with movement tracking
        if "stock_quantity" in medicine_data:
            old_stock = existing_medicine.get("stock_quantity", 0)
            new_stock = medicine_data["stock_quantity"]
            
            if old_stock != new_stock:
                # Create stock movement record
                stock_movement = StockMovement(
                    medicine_id=medicine_id,
                    medicine_name=existing_medicine.get("name", "Unknown"),
...
async def delete_medicine(medicine_id: str):
    ...
    try:
        ...
        # Check if medicine has been sold (optional safety check)
        sales_with_medicine = await db.sales.find_one({
            "items.medicine_id": medicine_id
        })
        
        if sales_with_medicine:
            # Instead of preventing deletion, we could mark it as discontinued
            # For now, we'll allow deletion but warn
            logging.warning(f"Deleting medicine {medicine_id} that has sales history")
        
        # Delete the medicine
        await db.medicines.delete_one({"id": medicine_id})
        
        # Create stock movement record for stock removal
        if medicine.get("stock_quantity", 0) > 0:
            stock_movement = StockMovement(
                ...
                new_stock=0,
                notes="Medicine deleted - stock removed"
            )
            await db.stock_movements.insert_one(stock_movement.dict())
...
async def get_sales(
    limit: int = Query(100, description="Number of sales to return"),
    skip: int = Query(0, description="Number of sales to skip")
):
    """Get all sales with pagination"""
    try:
        sales = await db.sales.find().sort("created_at", -1).skip(skip).limit(limit).to_list(limit)
        for sale in sales:
            sale.pop("_id", None)
...
async def create_sale(sale: Sale):
    ...
    try:
        ...
        for item in sale.items:
            ...
            if not medicine:
                raise HTTPException(status_code=400, detail=f"Medicine with ID {item['medicine_id']} not found")
            ...
            if sale.is_return:
                quantity_to_deduct = -abs(quantity_to_deduct)  # Returns add back to stock
            
            new_stock = current_stock - quantity_to_deduct
            
            # Validate stock availability for regular sales
            if not sale.is_return and new_stock < 0:
                raise HTTPException(
                    status_code=400, 
                    detail=f"Insufficient stock for {medicine['name']}. Available: {current_stock}, Requested: {quantity_to_deduct}"
                )
            ...
            # Create stock movement record
            movement_type = "return" if sale.is_return else "sale"
            stock_movement = StockMovement(
                medicine_id=item["medicine_id"],
                medicine_name=medicine["name"],
                movement_type=movement_type,
                quantity_change=-quantity_to_deduct,  # Negative for sales, positive for returns
                previous_stock=current_stock,
                new_stock=new_stock,
                reference_id=sale.id,
                notes=f"Sale/Return - Receipt: {sale.receipt_number}"
            )
            stock_movements.append(stock_movement.dict())
            
            # Update item with current medicine details
            updated_item = {
                **item,
                "medicine_name": medicine["name"],
                "unit_price": medicine.get("price", item.get("price", 0))
            }
            updated_items.append(updated_item)
        
        # Update sale with processed items
        sale.items = updated_items
...
async def get_sale(sale_id: str):
    ...
    try:
        ...
        logging.error(f"Error fetching sale: {e}")
        raise HTTPException(status_code=500, detail="Error fetching sale")

@api_router.put("/sales/{sale_id}", response_model=Sale)
async def update_sale(sale_id: str, sale_data: dict):
    """Update a sale (limited functionality for safety)"""
    try:
        existing_sale = await db.sales.find_one({"id": sale_id})
        if not existing_sale:
            raise HTTPException(status_code=404, detail="Sale not found")
        
        # For safety, only allow updating non-critical fields
        allowed_fields = ["customer_name", "customer_phone", "payment_method"]
        update_data = {}
        ...
        raise HTTPException(status_code=500, detail="Error updating sale")
    ...
    try:
        ...
        for item in sale.get("items", []):
            ...
            if medicine:
                ...
                if sale.get("is_return", False):
                    # If it was a return, we need to remove the stock again
                    new_stock = current_stock - quantity_to_restore
                else:
                    # If it was a sale, we need to add the stock back
                    new_stock = current_stock + quantity_to_restore
                ...
                stock_movement = StockMovement(
                    ...
                    medicine_name=medicine["name"],
                    movement_type="adjustment",
                    quantity_change=quantity_to_restore if not sale.get("is_return", False) else -quantity_to_restore,
                    previous_stock=current_stock,
                    new_stock=max(0, new_stock),
                    reference_id=sale_id,
                    notes=f"Sale deletion - Receipt: {sale.get('receipt_number', 'Unknown')}"
                )
                stock_movements.append(stock_movement.dict())
        ...
        raise HTTPException(status_code=500, detail="Error deleting sale")
