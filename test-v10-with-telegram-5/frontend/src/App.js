    ...
    const handleCreateBackup = async (e) => {
      ...
      try {
        ...
        if (result?.success) {
          ...
          if (activeTab === 'manage' || currentTab === 'manage') {
            await fetchBackups();
          }
            ...
            <div className="border-b border-slate-200">
              <nav className="flex p-6 space-x-8">
                {[
                  ...
                  >
                    {tab.label}
                    {(backupLoading && activeTab === 'create') && <span className="ml-2 inline-block w-3 h-3 border border-blue-600 border-t-transparent rounded-full animate-spin"></span>}
                    {(restoreLoading && activeTab === 'restore') && <span className="ml-2 inline-block w-3 h-3 border border-green-600 border-t-transparent rounded-full animate-spin"></span>}
                    {(uploadingFile && activeTab === 'manage') && <span className="ml-2 inline-block w-3 h-3 border border-yellow-600 border-t-transparent rounded-full animate-spin"></span>}
                  </button>
                ))}
              </nav>
            </div>
