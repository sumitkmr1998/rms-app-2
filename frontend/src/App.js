...
const RMSApp = () => {
  ...
  const [backupActiveTab, setBackupActiveTab] = useState('create'); // Move activeTab to parent
  ...
  useEffect(() => {
    if (user) {
      ...
      // Sync any existing offline data with backend
      syncLocalDataWithBackend();
    }
  }, [user]);
  ...
  // API utility function with fallback
  const makeApiCall = async (endpoint, options = {}) => {
    const externalUrl = process.env.REACT_APP_BACKEND_URL;
    const localUrl = 'http://localhost:8001';
    
    // Try external URL first
    try {
      const response = await fetch(`${externalUrl}${endpoint}`, options);
      if (response.ok || response.status < 500) {
        return response;
      }
      throw new Error('External URL not accessible');
    } catch (externalError) {
      console.warn('External API call failed, trying localhost:', externalError.message);
      
      // Fallback to localhost
      try {
        const response = await fetch(`${localUrl}${endpoint}`, options);
        return response;
      } catch (localError) {
        console.error('Both external and local API calls failed:', localError);
        throw localError;
      }
    }
  };
  ...
  const createBackup = async (options) => {
    try {
      ...
      const response = await makeApiCall('/api/backup/create', {
        method: 'POST',
        headers: {
      ...
      console.log('Backup create response status:', response.status);
      
      if (!response.ok) {
        const errorText = await response.text();
        console.error('Backup create error:', errorText);
        throw new Error(`Failed to create backup: ${response.status} ${errorText}`);
      }
      
      const result = await response.json();
      console.log('Backup create result:', result);
      return result;
    } catch (error) {
  ...
  const fetchBackups = async () => {
    try {
      ...
      const response = await makeApiCall('/api/backup/list');
      
      console.log('Fetch backups response status:', response.status);
      
      if (!response.ok) {
        const errorText = await response.text();
        console.error('Fetch backups error:', errorText);
        throw new Error(`Failed to fetch backups: ${response.status} ${errorText}`);
      }
      
      const data = await response.json();
      console.log('Fetch backups result:', data);
  ...
  const restoreBackup = async (options) => {
    try {
      ...
      const response = await makeApiCall('/api/backup/restore', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.log('Backup restore response status:', response.status);
      
      if (!response.ok) {
        const errorText = await response.text();
        console.error('Backup restore error:', errorText);
  ...
  const deleteBackup = async (backupId) => {
    try {
      const response = await makeApiCall(`/api/backup/${backupId}`, {
        method: 'DELETE',
      });
  ...
  const downloadBackup = async (backupId, backupName) => {
    try {
      const response = await makeApiCall(`/api/backup/download/${backupId}`);
      if (!response.ok) {
  ...
  const uploadBackupFile = async (file) => {
    try {
      ...
      const response = await makeApiCall('/api/backup/upload', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleEditMedicine = async (e) => {
    ...
    try {
      ...
      const updatedMedicine = await updateMedicine(selectedMedicine.id, medicineData);
        ...
        <div className="flex items-center gap-2">
          ...
          {(canAccessView('users') || canAccessView('shop') || canAccessView('backup') || canAccessView('telegram')) && (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${currentView === 'settings' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
            >
              ⚙️ Settings
            </button>
          )}
  ...
  const BackupRestoreManagement = () => {
    ...
    // Remove local activeTab state - use parent's backupActiveTab instead
    const [selectedBackupForRestore, setSelectedBackupForRestore] = useState(null);
    const [uploadingFile, setUploadingFile] = useState(false);
    
    // Store the active tab in a ref to prevent reset during operations
    const activeTabRef = useRef(backupActiveTab);
    useEffect(() => {
      activeTabRef.current = backupActiveTab;
    }, [backupActiveTab]);

    useEffect(() => {
      if (backupActiveTab === 'manage' || backupActiveTab === 'restore') {
        fetchBackups();
      }
    }, [backupActiveTab]);
    ...
    const handleCreateBackup = async (e) => {
      ...
      // Store current tab to prevent reset during backup operation
      ...
      try {
        ...
        if (result?.success) {
          setMessage(`✅ Backup created successfully! ${result.total_records || 0} records backed up.`);
          setBackupOptions({
            ...backupOptions,
            backup_name: ''
          });
          // Refresh backup list if on manage tab
          if (backupActiveTab === 'manage' || currentTab === 'manage') {
            await fetchBackups();
          }
          
          // Ensure tab stays where it was
          if (currentTab && currentTab !== 'create') {
            setBackupActiveTab(currentTab);
          }
        } else {
          setMessage('❌ Failed to create backup: Invalid response');
        }
      } catch (error) {
    ...
    const handleRestoreBackup = async (e) => {
      ...
      // Store current tab to prevent reset during restore operation
      const currentTab = activeTabRef.current;
      ...
      try {
        ...
        if (result?.success) {
          const totalRestored = result.restored_records ? 
            Object.values(result.restored_records).reduce((sum, count) => sum + count, 0) : 0;
          setMessage(`✅ Backup restored successfully! ${totalRestored} records restored.`);
          setSelectedBackupForRestore(null);
          
          // Refresh all data without changing tab
          await Promise.all([
            fetchMedicines(),
            fetchSales(),
            fetchShop()
    ...
    const handleDeleteBackup = async (backup) => {
      ...
      // Store current tab to prevent reset
      const currentTab = activeTabRef.current;
            ...
            <div className="border-b border-slate-200">
              <nav className="flex p-6 space-x-8">
                {[
                  ...
                  <button
                    key={tab.id}
                    onClick={() => setBackupActiveTab(tab.id)}
                    disabled={backupLoading || restoreLoading || uploadingFile}
                    className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                      backupActiveTab === tab.id
                        ? 'bg-blue-100 text-blue-700 border border-blue-200'
                        : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
                    } ${(backupLoading || restoreLoading || uploadingFile) ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    {tab.label}
                    {(backupLoading && backupActiveTab === 'create') && <span className="ml-2 inline-block w-3 h-3 border border-blue-600 border-t-transparent rounded-full animate-spin"></span>}
                    {(restoreLoading && backupActiveTab === 'restore') && <span className="ml-2 inline-block w-3 h-3 border border-green-600 border-t-transparent rounded-full animate-spin"></span>}
                    {(uploadingFile && backupActiveTab === 'manage') && <span className="ml-2 inline-block w-3 h-3 border border-yellow-600 border-t-transparent rounded-full animate-spin"></span>}
                  </button>
                ))}
              </nav>
            </div>
