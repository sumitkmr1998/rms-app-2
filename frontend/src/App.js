...
const RMSApp = () => {
  ...
  const canAccessView = (view) => {
    ...
    switch (view) {
      ...
      case 'notifications': // Notifications for admin and manager
  ...
  // Data fetching functions - Updated to handle async operations
  const fetchMedicines = async (search = '') => {
    try {
      const medicineList = await getMedicines(search);
      setMedicines(medicineList);
    } catch (error) {
      console.error('Error fetching medicines:', error);
      setMedicineMessage('Error fetching medicines');
    }
  };
  ...
  const fetchSales = async () => {
    try {
      ...
      setSaleMessage('Error fetching sales');
    }
  };
  ...
  const fetchShop = async () => {
    try {
      ...
      setShopMessage('Error fetching shop details');
    }
  };
  ...
  const handleEditSale = async (e) => {
    ...
    try {
      ...
      const updatedSale = await updateSale(selectedSale.id, updatedSaleData);
    ...
    <nav className="bg-slate-800 border-b border-slate-700 px-6 py-4">
      <div className="flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-2">
          ...
          {canAccessView('notifications') && (
            <button
              onClick={() => setCurrentView('notifications')}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${currentView === 'notifications' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
            >
              📱 Notifications
            </button>
          )}
    ...
    <div className="App">
      ...
      {!loading && isAuthenticated && (
        <>
          ...
          {currentView === 'notifications' && <TelegramNotifications />}
        </>
      )}
