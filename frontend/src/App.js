    ...
    <nav className="bg-slate-800 border-b border-slate-700 px-6 py-4">
      <div className="flex justify-between items-center max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-2">
          ...
          {canAccessView('telegram') && (
            <button
              onClick={() => setCurrentView('telegram')}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${currentView === 'telegram' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
            >
              📱 Telegram Alerts
            </button>
          )}
    ...
    <div className="App">
      ...
      {!loading && isAuthenticated && (
        <>
          ...
          {currentView === 'telegram' && <TelegramNotifications />}
        </>
      )}
